...
def apply_discount(request):
    ...
    return {'cart_items': cart_items, 'cumulative_discount': cumulative_discount, 'grand_total': grand_total}


def get_discounted_price(product):
    if product.discounts.exists() and get_discount_info(product)["valid_discount"] is not None:
        product.price *= (100 - get_discount_info(product)["discount_value"]) / 100
